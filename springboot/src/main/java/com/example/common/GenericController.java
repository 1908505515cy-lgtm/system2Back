...
public abstract class GenericController<E, D, V> {
    ...
    @PostMapping("/add")
    public Result add(@Valid @RequestBody D dto) {
        getService().add(dto);
        return Result.success("新增成功");
    }

    @PutMapping("/update")
    public Result update(@Valid @RequestBody D dto) {
    ...
    public Result page(@RequestParam(required = false) String keyword,
                       ...
                       @RequestParam(required = false) String _filter,
                       @RequestParam(required = false) String _logic) {
    ...
    @GetMapping("/trash")
    public Result trash(@RequestParam(required = false) String keyword,
                        @RequestParam(defaultValue = "1") Integer pageNum,
                        @RequestParam(defaultValue = "10") Integer pageSize) {
        if (pageNum < 1) pageNum = 1;
        if (pageSize < 1) pageSize = 10;
        if (pageSize > MAX_PAGE_SIZE) pageSize = MAX_PAGE_SIZE;
        PageResult<V> result = getService().trash(keyword, pageNum, pageSize);
        return Result.success(result);
    }
    ...
    public Result batchRestore(@RequestBody List<Long> ids) {
        ...
        getService().batchRestore(ids);
        return Result.success("批量恢复成功");
    }

    @PostMapping("/batch-permanent-delete")
    public Result batchPermanentDelete(@RequestBody List<Long> ids) {
        if (ids.size() > MAX_BATCH_SIZE) {
            throw new CustomException(ResultCodeEnum.PARAM_ERROR.getCode(),
                    "批量操作最多支持 " + MAX_BATCH_SIZE + " 条");
        }
        getService().batchPermanentDelete(ids);
        return Result.success("批量彻底删除成功");
    }
}
